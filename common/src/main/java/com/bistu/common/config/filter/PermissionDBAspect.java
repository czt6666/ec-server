...
import com.bistu.common.config.annotation.Logical;
import com.bistu.common.config.annotation.RequiresPermissions;
import com.bistu.common.config.exception.UnauthorizedException;
...
@Component
@Order(3)
public class PermissionDBAspect {
    @Autowired
    TokenUtil tokenService;

    private final static String REFRESH_PERMISSION = "refresh";

    @Before("@annotation(com.bistu.common.config.annotation.RequiresPermissions)")
    public void before(JoinPoint joinPoint) {
        ...
        if (Arrays.asList(perms).contains(REFRESH_PERMISSION)) {
            return;
        }
