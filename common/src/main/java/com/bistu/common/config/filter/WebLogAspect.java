...
public class WebLogAspect {
    @Pointcut("execution(public * com.bistu.*.controller..*.*(..))")
    public void webLog() {
    }

    @Pointcut(" execution(public * com.bistu.common.config.exception.GlobalExceptionHandler.*(..))")
    ...
    public void afterReturning(Object result) {
        ...
        if (map != null) {
            ...
            JSONObject reqObj = JSON.parseObject(map.getOrDefault("req", ""));
            if (reqObj != null && reqObj.containsKey("requestBody")) {
                String base64Str = reqObj.getJSONObject("requestBody").getString("base64");
                if (base64Str != null && base64Str.length() > 200) {
                    reqObj.getJSONObject("requestBody").put("base64", base64Str.substring(0, 20) + "...");
                }
            }
            jsonObject.put("req", reqObj);
//            if (result != null) {
